...
async function _send({ client, ctx, conversation, logger, message, ack }: SendMessageProps) {
  ...
  if (recipient.kind === 'bsuid') {
    logger
      .forBot()
      .info(
        `Sending WhatsApp ${messageType} via BSUID fallback (no userPhone tag on conversation; bsuid="${recipient.value}").`
      )
  }
  ...
  const feedback = await repeat(
    async (i) => {
      ...
      const result = await whatsapp.sendMessage(botPhoneNumberId, recipient.value, message)
