  ...
  actions: {
    ...
    startFlow: {
      ...
      output: {
        schema: z
          .object({
            messaging_product: z.literal('whatsapp').title('Messaging Product'),
            contacts: z
              .array(
                z.object({
                  input: z.string().title('Input'),
                  wa_id: z.string().title('WA ID'),
                })
              )
              .title('Contacts')
              .describe('Contacts acknowledged by WhatsApp'),
            messages: z
              .array(
                z.object({
                  id: z.string().title('Message ID'),
                })
              )
              .title('Messages')
              .describe('Messages created by WhatsApp'),
          })
          .title('Start Flow Response')
          .describe('Raw WhatsApp API response after sending the Flow'),
      },
    },
  },
  events: {
